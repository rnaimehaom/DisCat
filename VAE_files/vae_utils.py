...
import random
import yaml
from VAE_models import load_encoder, load_decoder, load_property_predictor
import numpy as np
import pandas as pd 
import os
from model_utilities import fast_verify

class VAEUtils(object):
    def __init__(self, exp_file='exp.json', encoder_file=None, decoder_file=None, directory=None):
        if directory is not None:
            curdir = os.getcwd()
            os.chdir(os.path.join(curdir, directory))

        self.params = parameters.load_params(exp_file, False)
    ...
    def enc_dec_functions(self, standardized=True):
        print(f'Using standarized functions? {standardized}')
